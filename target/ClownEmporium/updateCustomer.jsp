...
catch (java.lang.ClassNotFoundException e)
{
    ...
    PreparedStatement pst = conn.prepareStatement("UPDATE customer SET firstName = ?, lastName = ?, email = ?, phonenum = ?, address = ?, city = ?, state = ?, postalCode = ?, country = ?, userid = ?, password = ? WHERE userid = ?;")
    ){
        for(int i = 0; i < arr.length; i++){
            pst.setString(i + 1, "\'" + (arr[i]) + "\'");// set the ? in pst to each of the users input
        }
        pst.setString(12, "\'" + authenticatedUser + "\'");
        pst.execute();    
        conn.commit();
